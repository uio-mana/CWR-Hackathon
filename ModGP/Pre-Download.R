#' ####################################################################### #
#' PROJECT: [BioDT CWR - Pre-ModGP Data Download] 
#' CONTENTS: 
#'  - Downloading GBIF Data
#'  - Downloading ERA5-Land Bioclimatic Data
#'  - Downloading Posthoc Covariate Data
#'  DEPENDENCIES:
#'  - Rquires to be passed a two-word string called "Spec_Ident" denoting the species name for whhose genus to download GBIF data
#' AUTHOR: [Erik Kusch]
#' ####################################################################### #
...
package_vec <- c(
	...
	"pbapply" # for parallel executions with timer
)
...
PH_FILE <- file.path(Dir.Data.Envir, "PosthocCovariates.nc")
if(file.exists(PH_FILE)){
	message("Pohsthoc covariate data loaded from disk.")
	PH_stack <- rast(PH_FILE)
}else{
	...
	message("Pohsthoc covariate data retrieved online and saved to disk.")
	writeCDF(x = PH_stack, filename = PH_FILE)
}